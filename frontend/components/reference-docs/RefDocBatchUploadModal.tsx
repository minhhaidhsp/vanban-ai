...
type Visibility = "private" | "org" | "system";
...
interface RefDocBatchUploadModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}
...
const VISIBILITY_OPTIONS: { value: Visibility; label: string; desc: string }[] = [
  { value: "private", label: "Chỉ mình tôi", desc: "Chỉ bạn xem được" },
  { value: "org",     label: "Cơ quan",      desc: "Chia sẻ trong cơ quan" },
  { value: "system",  label: "Hệ thống",     desc: "Tất cả người dùng" },
];
...
export function RefDocBatchUploadModal({ open, onClose, onSuccess }: RefDocBatchUploadModalProps) {
  ...
  const startPolling = useCallback((initialJobs: Job[]) => {
    ...
    pollRef.current = setInterval(async () => {
      const results = await Promise.all(
        ids.map((id) => refDocApi.getJobStatus(id).catch(() => null))
      );
      setJobs((prev) => {
        const next = prev.map((j, i) => {
          const upd = results[i];
          return upd ? { ...j, status: upd.status, error: upd.error } : j;
        });
        if (next.every((j) => j.status === "done" || j.status === "failed")) {
          stopPolling();
          onSuccess?.();
        }
        return next;
  ...
  const handleUpload = async () => {
    ...
    setUploadError(null);
    try {
      const res = await refDocApi.uploadBatch(selectedFiles, visibility);
      const initialJobs: Job[] = res.jobs.map((j) => ({
        job_id: j.job_id, filename: j.filename, status: "pending", error: null,
      }));
      setSelectedFiles([]);
      setJobs(initialJobs);
  ...
  const allSettled = jobs.length > 0 && jobs.every(
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o && !isUploading) onClose(); }}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Upload văn bản tham chiếu</DialogTitle>
        </DialogHeader>
          ...
          <>
            ...
            >
              <input ref={fileInputRef} type="file" className="hidden"
                accept={ACCEPT} multiple onChange={handleFileChange} />
            ...
            <p className="text-xs text-muted-foreground text-center -mt-1">
              ✨ AI sẽ tự động trích xuất số/ký hiệu, trích yếu và cơ quan ban hành
            </p>

            {/* Visibility selector */}
            <div>
              <label className="text-sm font-medium mb-1 block">Phạm vi chia sẻ</label>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={onClose} disabled={isUploading}>Hủy</Button>
          ...
          <>
              ...
              {jobs.map((job) => (
                <div key={job.job_id}
                  ...
                  <span className={cn(
                    ...
                    (job.status === "pending" || job.status === "processing") && "text-muted-foreground",
                  )}>
                    {statusLabel[job.status]}
                  </span>
                </div>
            ...
            <DialogFooter>
              <Button onClick={onClose} disabled={!allSettled}>
                {allSettled ? "Đóng" : (
                  <><Loader2 className="h-4 w-4 animate-spin mr-1" />Đang xử lý...</>
                )}
              </Button>
            </DialogFooter>
          </>
